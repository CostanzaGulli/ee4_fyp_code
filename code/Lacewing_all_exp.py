...
if __name__ == "__main__":
    ...
    if excel:
        ...
        for exp_path in tqdm(experiments):
            ...
            out = la.algo(exp_path, visualise_preprocessing=True, save_preprocessing=True, visualise_processing=True, save_processing=True)
            ...
            if positive_pred[0][positive_pred.shape[0]-2] != positive_true[0][positive_true.shape[0]-2]:
                print(f'!!!!!!!!!!!top well wrong. exp {experiment_id}')
            if positive_pred[0][positive_pred.shape[0]-1] != positive_true[0][positive_true.shape[0]-1]:
                print(f'!!!!!!!!!!!bot well wrong. exp {experiment_id}')
        ...
        # for i in range(all_qubit.shape[0]):
        #     if positive_true[0][i] == 'positive':
        #         if all_fiterror[0][i] != -2:
        #             ax.scatter(all_qubit[0][i], all_fiterror[0][i], c='r', linewidth=2)
        #     else:
        #         if all_fiterror[0][i] != -2:
        #             ax.scatter(all_qubit[0][i], all_fiterror[0][i], c='k', linewidth=2)
        # ax.set_title('Maximum fitting error at inflection point', fontsize=20)
        # ax.set_xlabel('Qubit value', fontsize=18)
        # ax.set_ylabel('Max fit error at infl', fontsize=18)
        # # ax.set(title='Max error Max error at inflection point', xlabel='Qubit value', ylabel='Max fit error')
        # plt.savefig('attempt3.eps')
        # plt.show()
